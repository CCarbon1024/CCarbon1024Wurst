...
{
	private final CheckboxSetting LeaveInSingle = new CheckboxSetting("Single Mode" ,
					"Make you leave with only one person",
			true);

	private final SliderSetting MinTotems = new SliderSetting("Totems",
			"Leaves the server when your totems numbers\n"
					+ "reaches this value or falls below it.\n"
					+ "Effective only when you using AutoTotem",
	...
	public AutoLeaveHack()
	{
		...
		addSetting(LeaveInSingle);
		addSetting(MinTotems);
		addSetting(health);
		addSetting(mode);
	}
	...
	public void onUpdate()
	{
		...
		if(!LeaveInSingle.isChecked() && MC.isSingleplayer()
			&& MC.player.connection.getPlayerInfoMap().size() == 1)
			return;

		if(WURST.getHax().autoTotemHack.isEnabled() &&
				WURST.getHax().autoTotemHack.getTotemsNumber() > MinTotems.getValueF()) {
			return;
		}
		...
		switch(mode.getSelected())
		{
			...
			MC.player.connection
				.sendPacket(new CChatMessagePacket("\u00a7"));
			break;
			
			case TELEPORT:
			MC.player.connection.sendPacket(
				new CPlayerPacket.PositionPacket(3.1e7, 100, 3.1e7, false));
			break;

			case SELFHURT:
			MC.player.connection
				.sendPacket(new CUseEntityPacket(MC.player,
